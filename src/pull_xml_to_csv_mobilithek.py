...
def pull_and_save_data(working_directory = os.getcwd()):
    try:
        ...
        if response.status_code == 200:
            ...
            mdm = pdx.read_xml(response.text,['soapenv:Envelope','soapenv:Body','d2LogicalModel','payloadPublication','siteMeasurements'],root_is_rows=False)
            ...
            for idx, row in mdm.iterrows():
                ...
                if len(row.measuredValue) == 3:
                    temp_Flow = int(row.measuredValue[0]['measuredValue']['basicData']['vehicleFlow']['vehicleFlowRate'])
                    ...
                    temp_veh_type_flow = row.measuredValue['measuredValue']['basicData']['forVehiclesWithCharacteristicsOf']['vehicleType']
                    temp_speed = 0
